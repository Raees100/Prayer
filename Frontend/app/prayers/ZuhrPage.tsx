...
const ZuhrPage = () => {
    ...
    const [prayerCompleted, setPrayerCompleted] = useState<boolean>(
      Array.isArray(isCompleted) ? isCompleted[0] === "true" : isCompleted === "true");
        ...
        const fetchPrayerStatus = async () => {
          try {
            const response = await prayerApi.getPrayerByType("Zuhr", currentDate);
            if (response) {
  ...
  const swipeLeft = Gesture.Fling()
    ...
    .onEnd(() => {
      ...
      if (currentPrayers.length > 2) {
        navigateToPrayerPage(currentPrayers[2]); 
      }
    });
  ...
  <PrayerStatusPage
    prayerName="Zuhr"
    isCompleted={prayerCompleted} // Convert to boolean
